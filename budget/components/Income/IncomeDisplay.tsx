...
export default async function IncomeDisplay(props: { budgetsGroups: BudgetGroup[], income: number, month: number, year: number, colors: HSLColor[] }) {
    ...
    const availablePercentage = ((props.income - totalAllocated.totalSpent) / props.income) * 100;
    ...
    return (<Card className="w-fit mt-4 pt-4">
        <CardContent className="flex flex-row items-center gap-4">
            ...
            <div className={`font-bold text-sm ${totalAllocated.totalGoal > props.income ? "text-red-500" : ""}`}>Budgeted</div>
            ...
            <div>
                ...
                <div className="flex flex-row gap-2">
                    <div className="font-bold text-xs">Free to Allocate</div>
                    <div className={`font-bold text-xs ${totalAllocated.totalGoal < 0 ? "text-red-500" : "text-gray-400"}`}>{parseFloat((100 - allocatedPercentage).toFixed(2))}%</div>
                </div>
            ...
            <div>
                <div className="flex flex-row place-content-end gap-2">
                    ...
                    <div className={`font-bold text-sm ${totalAllocated.totalSpent < 0 ? "text-red-500" : "text-gray-400"}`}>{parseFloat(availablePercentage.toFixed(2))}%</div>
                </div>
                <div className="flex flex-row place-content-end gap-2">
                    <div className="font-bold text-sm">Spent</div>
                    <div className={`font-bold text-sm ${totalAllocated.totalSpent > props.income ? "text-red-500" : "text-gray-400"}`}>{parseFloat((100-availablePercentage).toFixed(2))}%</div>
                </div>

                <IncomeSpent income={props.income} month={props.month} year={props.year} colors={props.colors} />
            </div>
            <div className="flex flex-col">
            <div className={`font-bold text-sm ${totalAllocated.totalGoal > props.income ? "text-red-500" : ""}`}>Remaining</div>
                <div className={`font-bold text-2xl ${totalAllocated.totalSpent > props.income ? "text-red-500" : ""}`}>{props.income - totalAllocated.totalSpent}€</div>
                <div className="font-bold text-gray-400 text-sm">of {props.income}€</div>
            </div>
        </CardContent>
    </Card>)
