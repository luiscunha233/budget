...
import { calculateBudgetGroupTotals, getAllBudgetGroupsService } from "@/lib/service/BudgetGroupService";
...
export default async function IncomeAllocations(props: { income: number, month: number, year: number, colors : HSLColor[] }) {
    ...
    for (let i = 0; i < allBudgetGroups.length; i++) {
        ...
        allocationByBudgetGroup.push(<div className="h-5 z-1" style={{ width: `${(totalOfBudgets.totalSpent / props.income) * 100}%`
